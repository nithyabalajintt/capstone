...
class DotDict(dict):
    """Allows accessing dictionary keys as attributes, with safe defaults."""
    def __getattr__(self, key):
        if key in self:
            return self[key]
        else:
            # ✅ Default values for commonly missing keys
            defaults = {
                "flows": [],         # Empty list if 'flows' is missing
                "bot_messages": {},  # Empty dict if 'bot_messages' is missing
            }
            return defaults.get(key, None)  # Return default if available, else None
    ...
    @classmethod
    def from_dict(cls, obj):
        """Recursively convert a dictionary to DotDict, but keep lists as lists"""
        if isinstance(obj, dict):
            return cls({k: cls.from_dict(v) for k, v in obj.items()})
        elif isinstance(obj, list):
            return [cls.from_dict(i) if isinstance(i, dict) else i for i in obj]  # ✅ Keep lists as lists
        return obj
...
config_obj.flows = config_obj.get("flows", [])  # Ensure it's a list
config_obj.bot_messages = config_obj.get("bot_messages", {})  # Ensure it's a dict
 
# ✅ Print to confirm
print(config_obj.get("colang_version", "No colang_version found"))
print(type(config_obj.flows))  # Should print <class 'list'>
print(config_obj.flows)  # Should be a list
print(config_obj.bot_messages)  # Should be a dict
